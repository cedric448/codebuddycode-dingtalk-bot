...
### v1.4.0 (2026-04-04)
**系统性优化：健壮性、性能、用户体验和代码质量**

#### 🔒 P0 健壮性优化
- **消息去重增强**：LRU 缓存（OrderedDict 实现，O(1) 复杂度）自动淘汰，防止内存无限增长
- **异常处理规范化**：所有异常统一使用 `exc_info=True` 自动记录堆栈，移除手动 `traceback.format_exc()`
- **后端错误重试增强**：HTTP 500/502/503/504 自动重试，优化等待时间（500: 2s, 502: 3s, 503: 2s, 504: 5s）
- **用户友好错误提示**：隐藏技术细节，提供清晰的用户层错误消息（超时、网络、服务不可用）
- **关键 Bug 修复**：修复 `codebuddy_client.py` 缺失 `import requests` 导致的 NameError

#### ⚡ P1 性能优化
- **HTTP 连接池复用**：全局 `requests.Session` 持久化连接，避免重复建连开销
- **正则表达式预编译**：Markdown 检测正则在类加载时编译，避免运行时重复编译
- **异步任务定期清理**：3600s 定时清理超过 24h 的历史任务，释放内存
- **二分查找压缩**：图片压缩使用二分搜索最优质量（20-85 范围），减少网络传输

#### 🎯 P1 用户体验优化
- **消息模板统一**：新增 5 个 `MSG_*` 常量，所有用户消息使用模板而非硬编码字符串
- **错误消息分级**：根据异常类型（Timeout/ConnectionError/HTTPError）提供针对性提示
- **增强日志上下文**：所有错误日志包含 URL、重试次数、响应摘要等调试信息

#### 🧹 P2 代码重构
- **方法提取去重**：提取 `_send_webhook_message()` 通用方法，5 个 reply_* 方法重用，减少 120+ 行重复代码
- **静态方法分离**：创建 `_user_friendly_error()` 静态方法，集中管理错误映射逻辑
- **代码行数优化**：bot.py 从 1084 行降至 963 行（-11%），提高可维护性

#### 📊 技术指标
- **消息去重**：LRU 最大 10000 条，自动淘汰策略
- **重试策略**：超时 3 次指数退避（2^attempt，最大 10s），HTTP 错误 2 次快速重试
- **日志增强**：100% 异常处理覆盖 exc_info=True，保留完整堆栈信息
- **代码复用**：消除 5 处重复 HTTP 发送逻辑，单一责任原则

#### 🔧 部署说明
本次更新无需修改配置文件，直接重启服务即可生效：
```bash
